    ...
    try {
        if (argv.name) {
            if (argv.logic && argv.ui) {
                Promise.all([WriteFile(HTMLFile, 'text/html', 'html'), WriteFile(javaScriptFile, 'Javascript', 'js'), WriteFile(CSSFile, 'CSS', 'css'), WriteFileTest(testFile, 'Javascript', 'test.js'), WriteFileTestSinon(testFileSinon, 'Javascript', 'sinon.js')])
                    .then((values) => {
                ...
                Promise.all([WriteFile(javaScriptFile, 'Javascript', 'js'), WriteFileTest(testFile, 'Javascript', 'test.js'), WriteFileTestSinon(testFileSinon, 'Javascript', 'sinon.js')])
                    .then((values) => {
                        console.log(chalk.green('Create Templates Successfull'));
                    }).catch((error) => {
                        console.log(chalk.red('Did not create the templates'));
...
const createDirectory = (argv) => {
    ...
    Promise.all([ReadFile('templateHTML', argv.name), ReadFile('templateJS', argv.name), ReadFile('templateCSS', argv.name), ReadFile('templateTEST', argv.name), ReadFile('templateSINON', argv.name), WriteDirectory(argv.name)])
        .then((values) => {
