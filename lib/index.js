...
const WriteDirectory = (firstDir,name) => {
    return new Promise((resolve, reject) => {
        filesSystem.mkdir(`${process.cwd()}/${firstDir}/${name}`, (error, data) => {
            if (error){
                reject(error);
            }
...
module.exports = { createDirectory, WriteFile, WriteDirectory, ReadFile, WriteFileTest, WriteFileTestSinon, ReplaceString, sum};
